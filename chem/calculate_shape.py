...
def main():
    ...
    parser.add_argument("-o", "--output", type=str, default='shapesimmat.txt', help="Similarity matrix")
    ...
    parser.add_argument("--nconfs", type=int, default=25, help="Number of conformers per compounds")
    parser.add_argument("--prunerms", type=float, default=0.5, help="Conformer prune RMS")
    parser.add_argument("--nthreads", type=int, default=1, help="Number of threads to use ('-1' for all threads)")
    ...
    if args.method not in ["crippen", "mmfp"]:
        print(f"[FATAL] Method {args.method} not available! Try with either 'crippen' or 'mmfp'")
        exit(1)
...
# Conformer generation
    ...
    ref_mols = [
        Chem.AddHs(m)
        for m in Chem.SmilesMolSupplier(args.input[0], delimiter=args.delim)
        if m != None
    ]
    for mol in tqdm(ref_mols, desc=f"Calculating {args.nconfs} conformers for reference molecules"):
    ...
    if len(args.input) == 1:
        ...
        query_mols = [
            Chem.AddHs(m)
            for m in Chem.SmilesMolSupplier(args.input[1], delimiter=args.delim)
            if m != None
        ]
        for mol in tqdm(query_mols, desc=f"Calculating {args.nconfs} conformers for query molecules    "):
            AllChem.EmbedMultipleConfs(
                mol,
                clearConfs = True,
                numConfs = args.nconfs,
...
# Alignment and similarity calculation
    ...
    match args.method:
        ...
        case "crippen":
            ...
            query_params = [rdMolDescriptors._CalcCrippenContribs(mol) for mol in query_mols]
    ...
    if len(args.input) == 1 and (args.alpha == args.beta):
        for i, ref in tqdm(list(enumerate(ref_mols)), desc="Calculating Shape similarity"):
                ...
                if ref == query:
                    ...
                    all_tanimotos = pool.starmap(
                        ...
                        zip(
                            ...
                            itertools.product(range(ref.GetNumConformers()), range(query.GetNumConformers())),
    ...
    print()
    print(":: Shape similarity description ::")
    print("==================================")

    print(f"{'Mean ± SD'.ljust(15)} = {np.nanmean(simmat.flatten()):.3f} ± {np.nanstd(simmat.flatten()):.3f}")
    print(f"{'Median (IQR)'.ljust(15)} = {np.nanmedian(simmat.flatten()):.3f} ({np.nanpercentile(simmat.flatten(), 25):.3f} - {np.nanpercentile(simmat.flatten(), 75):.3f})")
