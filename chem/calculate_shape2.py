...
Based on https://iwatobipen.wordpress.com/2018/09/26/3d-alignment-function-of-rdkit-rdkit/
"""
def Crippen_ShapeTversky(ref_query, params, alpha, beta):
    ref, query = ref_query
    ref_id, ref_mol, ref_conf = ref
    query_id, query_mol, query_conf = query

    # Align molecules
    pyO3A = rdMolAlign.GetCrippenO3A(
        ref_mol, query_mol, params[0][ref_id], params[1][query_id], ref_conf, query_conf
    )
    pyO3A.Align()

    # Calculate shape similarity
    similarity = rdShapeHelpers.ShapeTverskyIndex(
        ref_mol, query_mol, alpha, beta, ref_conf, query_conf
    )

    return ref_id, query_id, similarity
...
def main():
    ...
    if len(args.input) == 1 and (args.alpha == args.beta):
        ...
        for i,ref_mol in enumerate(ref_mols):
            for j, query_mol in enumerate(query_mols[0:i+1]):
                for ref_conf in range(ref_mol.GetNumConformers()):
                    for query_conf in range(query_mol.GetNumConformers()):
                        comparisons.append(
                            ((i, ref_mol, ref_conf),
                            (j, query_mol, query_conf))
                        )
        ...
        for i,j,similarity in pool.starmap(
            ...
            zip(
                comparisons,
                itertools.repeat((ref_params, query_params)),
